...
check() {
	...
	require_binaries umount || return 1
        require_binaries bmap-writer || return 1
        require_binaries cryptsetup || return 1
        require_binaries openssl || return 1
        require_binaries find || return 1
...
install() {
        ...
        #inst_multiple mount umount cat cmp grep mkdir expr chroot cryptsetup openssl
        inst_multiple mount umount cat cmp grep mkdir expr chroot openssl bmap-writer find cryptsetup
    ...
    cp $moddir/init-extra.sh $initdir/init
}
